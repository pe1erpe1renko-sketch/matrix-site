...
/**
 * Тексты информационных блоков под формой — по одному набору на тип
 * разбора. Ключ = slug типа из CALC_TYPES: добавили тип — добавьте сюда
 * набор, иначе главная откроется без объясняющих блоков.
 */
const TABS = {
  matrica: {
    ...
    opens:
      "Двенадцать сфер, девяносто два вопроса, около шестидесяти чисел из одной даты. Октаграмма с восемью внешними точками и центром, чакральная таблица в трёх измерениях, четыре уровня предназначения, родовые линии, возрастная шкала на 80 лет.",
  ...
  sovmestimost: {
    ...
    note: "Расчёт работает для любой пары: партнёры, друзья, коллеги.",
  },
  dengi: {
    ...
    opens:
      "Денежный канал, способ его раскрытия, финансовый блок, отношения со статусом, профессиональный талант и сфера реализации — двадцать четыре вопроса.",
    noteTitle: "Важно",
    note: "Сумм и сроков здесь нет. Матрица показывает механизм, а не прогноз доходов.",
  },
  "mama-rebenok": {
    lead: "Узнайте, что происходит между вами и ребёнком и как к нему подойти",
    understand: [
      "что вас связывает и какая задача у вас общая",
      "что открывает ребёнка, а что вызывает сопротивление",
      "в какой точке вы регулярно упираетесь друг в друга",
      "какие родовые программы идут через вас обоих",
    ],
    opens:
      "Ядро связи, общая задача, подход и точки трения, родовые линии, общие для вас двоих.",
    noteTitle: "Считается по двум датам",
    note: "Первая — ваша, вторая — ребёнка. Это карта особенностей, а не заключение о развитии.",
  },
  detskaya: {
  ...
  karma: {
    lead: "Узнайте, какой урок возвращается к вам, пока вы его обходите",
    understand: [
      "какая задача стоит перед вами и почему её не получается обойти",
      "что тянется из прошлого и в каких ситуациях это включается",
      "какие события повторяются и что их запускает",
      "что из этого пришло по роду, а что вы завели сами",
    ],
    opens:
      "Кармическая задача, долг, повторяющиеся события, урок, который вы обходите, и обе линии рода — семнадцать вопросов.",
    noteTitle: "Не приговор",
    note: "Матрица показывает механизм, а не наказание. Урок — это то, чему можно научиться, а не то, что с вами сделали.",
  },
  zdorovie: {
    lead: "Узнайте, где тело сдаёт первым, когда вы перегружены",
    understand: [
      "какие зоны просят внимания при перегрузке",
      "как ваше тело реагирует на стресс",
      "куда утекает энергия и что вызывает выгорание",
      "что вас восстанавливает быстрее всего",
    ],
    opens:
      "Чакральная таблица в трёх измерениях, зоны внимания, реакция на стресс, источники энергии и способы восстановления.",
    noteTitle: "Это не диагноз",
    note: "Матрица показывает зоны внимания. При недомогании обращайтесь к врачу — расчёт этого не отменяет и не заменяет.",
  },
  rod: {
    lead: "Узнайте, что тянется от родителей и что можно остановить на себе",
    understand: [
      "что идёт по мужской линии, а что по женской",
      "какие программы вы приняли, не заметив этого",
      "на какой дар рода можно опереться",
      "что вы передаёте детям прямо сейчас",
    ],
    opens:
      "Обе линии рода с промежуточными программами, родовой аркан, дары и то, что стоит остановить на себе, плюс детско-родительские связи.",
    noteTitle: "Про живых людей",
    note: "Расчёт говорит о программах, а не о вине родителей. Программу можно увидеть и не передавать дальше.",
  },
  prednaznachenie: {
    lead: "Узнайте, что вам дано сделать и почему без этого не отпускает",
    understand: [
      "какая задача стоит перед вами лично и какая — в социуме",
      "чем отличается предназначение до сорока лет и после",
      "какие таланты просят, чтобы их раскрыли",
      "в каком деле вы вкладываете мало, а получаете много",
    ],
    opens:
      "Четыре уровня задачи: личная, социальная, духовная и планетарная. Плюс таланты, стиль работы и сфера реализации.",
    noteTitle: "Как считается",
    note: "Планетарное предназначение — сумма духовной гармонии и социализации. Это арифметика, а не откровение.",
  },
  biznes: {
...
const FAQ = [
  ...
  },
  {
    ...
    a: "Октаграмму со всеми числами, чакральную таблицу, предназначения, родовые программы и пятнадцать вопросов разбора из девяноста двух — по одному в каждой сфере. Без регистрации. Остальные вопросы, PDF, аркан дня и ИИ-наставник открываются на платном тарифе.",
  },
  {
    q: "Зачем нужны имя и пол, если на числа они не влияют?",
    a: "Имя подписывает разбор в кабинете: когда у вас несколько разборов — свой, партнёра, ребёнка — вы не перепутаете, где чей. Пол используется в детской матрице и в обращениях. На расчёт ни то ни другое не влияет, формула одна.",
  },
  {
        ...
        <div style={{ ...S.heroGrid, ...(isPhone ? { gridTemplateColumns: "minmax(0, 1fr)", gap: 30 } : null) }}>
          ...
          <div style={isPhone ? { order: 2 } : null}>
            ...
            <div style={S.stats}>
              ...
              <Stat n="92" l="вопроса из одной даты" />
          ...
          <div style={{ ...S.form, ...(isPhone ? { order: 1, padding: "20px 16px 22px" } : null) }}>
            ...
            {/* Одиннадцать типов в одну строку: карусель листается
                пальцем и мышью. Порядок задан в CALC_TYPES — по спросу. */}
            <CalcCarousel active={tab} />

            <Person data={a} set={setA} idPrefix="a"
              label={calc.pairs ? PAIR_LABELS[tab][0].block : null}
              dateLabel={calc.pairs ? PAIR_LABELS[tab][0].date : "Дата рождения"} />
            {calc.pairs && (
              <Person data={b} set={setB} idPrefix="b"
                label={PAIR_LABELS[tab][1].block}
                dateLabel={PAIR_LABELS[tab][1].date} />
            )}
        ...
        <div style={S.flowWrap}>
          ...
          <div style={S.flow}>
            <Flow n="1" title="Введите данные" art={<ArtForm />}
              text="Имя, дата рождения, пол. Имя подписывает разбор в кабинете — вы будете хранить несколько разборов и не перепутаете, где чей."
              meta="15 секунд" />
            ...
            <Flow n="3" title="Ядро открыто" art={<ArtFree />}
              text="Октаграмма, карта здоровья, предназначения и по одному вопросу в каждой сфере. Бесплатно и без регистрации."
              meta="15 из 92 вопросов" />
            <Flow n="4" title="Полный разбор" art={<ArtFull />}
              text="Остальные семьдесят семь вопросов и все десять типов разбора по этой дате, PDF, аркан дня каждое утро и ИИ-наставник, который знает вашу матрицу."
              meta="от 490 ₽" gold />
          </div>
        </div>
...
/**
 * Подписи полей у парных типов. У «Мамы и ребёнка» они обязаны быть
 * явными: перепутанные местами даты дадут ту же матрицу пары, но текст
 * будет читаться наоборот — про ребёнка как про взрослого.
 */
const PAIR_LABELS = {
  sovmestimost: [
    { block: "Первый", date: "Дата рождения" },
    { block: "Второй", date: "Дата рождения" },
  ],
  "mama-rebenok": [
    { block: "Родитель", date: "Дата рождения мамы или папы" },
    { block: "Ребёнок", date: "Дата рождения ребёнка" },
  ],
  biznes: [
    { block: "Первый партнёр", date: "Дата рождения" },
    { block: "Второй партнёр", date: "Дата рождения" },
  ],
};

function Person({ data, set, label, dateLabel = "Дата рождения", idPrefix }) {
  ...
  return (
    <div style={{ marginTop: label ? 16 : 14 }}>
      ...
      <label style={S.fieldLabel}>{dateLabel}</label>
